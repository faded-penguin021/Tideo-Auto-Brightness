...
if [ "${shizuku_sites:-0}" != "$shizuku_expected" ]; then
	printf 'doc-fact drift: %s file(s) reference ShizukuShell but the docs claim exactly %s runtime places — update the claim in the constitution (CLAUDE.md today; AGENTS.md once the pointer swap lands) and its restatements in README.md + docs/rebuild/architecture/privilege_tiers.md, the d66de4c drift sites, AND this guard'"'"'s constant in lockstep (DA-015)\n' \
