...
>
> That list is the whole of what `guard_state_size` and `guard_state_structure` check. Those
> functions upgrade independently of this file and are the authority: if a later harness adds a
> rung, this paragraph is what goes stale, and nothing checks it against the script. Everything
...
train. Post-release v1.9.1 work sits on `claude/first-work-item-plan-lqklk4`: all six items of
`docs/plans/REVIEW_TRIAGE_1.9.0.md` done (DB-065…DB-068), one commit each, ladder green. Nothing is
half-done; no review is owed. To ship v1.9.0: squash-merge #117, publish from the GitHub UI, then
**delete `docs/rebuild/DEVICE_TEST_SCRIPT_1.9.0.md`** (ephemeral), folding its results into the
standing script. Do **not** re-open the closed force-stop investigation — read DB-051…DB-060 first.
...
3. **Provenance manifest: one record removed (`ProfilesScreen.kt`), one removed then restored
   (`MiscScreen.kt`).** 14 no-coordinate records; hand-edited, never regenerated (DB-032).
...
5. **v1.9.1 device checks. Owner:** (a) Developer Options → Stay awake on, Apply in Privileged
   Display, then `settings get global stay_on_while_plugged_in` still reads 15 (DB-065); (b) on a
   device showing a non-AOSP color-correction mode, the picker carries the preservation notice and
   an unrelated Apply leaves the mode alone (DB-066).
...
- 2026-08-18 — **v1.9.1 triage items 1–6 done (DB-065…DB-068).** Stay-awake writes AOSP's full
  ...
  review's canonical-7 test was **not** adopted (it would re-run DB-049). Per-item detail and the
  severity disagreements: `docs/plans/REVIEW_TRIAGE_1.9.0.md`.
- 2026-08-17..18 — **Harness legislation reviewed to a close (DB-062…DB-064); location and Contexts
  ...
  that found eleven further defects, each fixture mutation-proved (suite 108 → 111); cancelled fixes
