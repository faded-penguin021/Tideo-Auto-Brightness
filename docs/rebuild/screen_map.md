...
| **Privileged Display** (`privileged_display`) | — rebuild-only feature, **no Tasker source** (D-149): AOSP display toggles behind WRITE_SECURE_SETTINGS (capability-gated Night Light + Kelvin and AOD, daltonizer/inversion, stay-awake-charging, and Android-14+ **Disable HDR (experimental)**). The HDR control writes stored format preferences, not the Force-SDR service API; apply/clear may need reboot and mode changes may briefly blank the screen (DB-044). Menu row only at ELEVATED; the screen self-guards below ELEVATED with a 3-channel grant card. ALL toggles are draft-edited `AabSettings` **profile fields** (D-151/D-152), applied on profile change by `DisplayTogglesCoordinator`, or directly on Apply while the service is off. Scheduling is a Contexts rule loading a profile. Optional "Follow circadian scaling" (D-154) rides the task90 tanh modifier. On open/resume the draft is seeded from device read-back (DB-034), skipped only when a DISPLAY field carries a user edit — not while `dirty`, which the merge's own write sets and which froze tracking after one change (DB-039/DB-040) — excluding temperature while circadian tracking owns it, and invalidated by Apply on both the direct and coordinator paths (DB-047/DB-048) | — |
