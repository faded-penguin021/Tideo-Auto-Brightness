...
| **Privileged Display** (`privileged_display`) | — rebuild-only feature, **no Tasker source** (D-149): AOSP display toggles behind WRITE_SECURE_SETTINGS (Night Light + Kelvin, daltonizer/inversion, AOD, stay-awake-charging, experimental HDR force-SDR). Menu row only at ELEVATED (own "Privileged" group); the screen self-guards below ELEVATED with a 3-channel grant card. ALL toggles are draft-edited `AabSettings` **profile fields** (D-151/D-152 — one set of controls, no device-immediate duplicates), applied on profile change by `DisplayTogglesCoordinator`, or written directly on Apply while the service is off. There is NO separate schedule feature: scheduling = a Contexts rule loading a profile that carries display fields. Optional per-profile "Follow circadian scaling" (D-154): the temperature rides the task90 tanh modifier via a coordinator ticker. On open and on every resume the draft is seeded from a **device read-back** (DB-034, Tasker `_ShowPrivilegedScene` parity) so the controls show what Android actually holds — skipped while the draft is dirty, and the temperature is excluded while circadian tracking owns that key | — |
