...
- **S13 — UI finalization (a–d, Opus/high), IN PROGRESS.** **S13a DONE** (design-system foundation:
  ...
  by S13a tokens & §2.5 blueprints; 10 instantiation tests; D-078). **Next: S13c** (apply the library
  screen-by-screen, check off m3_audit §3 rows) and then **S13d** (charts + About/User-Guide).
...
Append new entries as D-079, … with which segments they affect.
