...
S12.9e]; `LiveRuntimeState` staleness gate + Dashboard amber banner + 5s reset watchdog, D-075). NEXT: S12.9e —
concurrency safety & controller decomposition (`AppProcessScope`/`goAsync`, kill the `lateinit` controller cycle,
volatile audit, split the 596-LOC controller; RUNBOOK §S12.9e, D-069).** S12.9 lands repo hygiene
...
Append new entries as D-076, … with which segments they affect.
