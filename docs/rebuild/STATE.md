...
- **D-083 (S13d' — owner chart-fidelity review; the ChartCanvas fence was LIFTED with explicit owner
  ...
  **Decisions:**
  - **Swipe vs scrub:** the `ChartPager` swipe and the chart's interactive drag-scrub both want horizontal
    drags → conflict. Paged charts set `interactive=false` (swipe wins; the standalone `BrightnessCurveChart`
    on Curve & Brightness keeps `interactive=true`). This also removed the mis-rounded scrub readout the owner
    saw (1.1 vs 1.15) — values are now read off the axis ticks (2-dp for sub-10 ranges).
