...
|---|---|---|---|---|---|
| Exported `ControlReceiver` | No side effect before the opt-in gate, except a debug flash the user opted into (DB-035) | `externalControlEnabled` read first; unknown verbs refused before the admission gate; the gate-off flash requires debug level 8 and builds nothing at any other level | `ControlReceiverAdmissionTest`, `ControlReceiver.KNOWN_ACTIONS`, `ControlReceiverTest.gateOff_{flashesTheReason,isSilentAtEveryOtherDebugLevel}` (the pair — the silence test alone also passes with the feature reverted) | Once ON, **any installed app** may send every verb — see product risk below. At level 8 any app can raise one toast per command, and picks its text for `LOAD_PROFILE` (clamped to 40 chars, control/bidi stripped). A rejected broadcast now costs one extra in-memory `settingsDataStore` read to learn the level, held inside the admission gate | code+test |
