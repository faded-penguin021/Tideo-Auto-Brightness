...
**C:** S10 ∥ S11 (after S9b).
Serial spine: S0 → A → S4 → S5 → B → S9a → S9b → C → S12 → **S12.5a → S12.5b → S12.5c** → S13 → S14.
(S9 was split into S9a/S9b in S3.6 — see D-027. S12.5 added after Gate 2 — see D-045; Gate 2 is
re-tested after S12.5c, NOT after S12.)
...
**Model:** Haiku / high · **Size:** medium · **Preconditions:** S12.5 DONE (template + faithful screens exist),
...
**GATE 2 — surfaces & tiers (after S12.5; first attempt after S12 FAILED — D-045).** Full UI walkthrough: every field edits/persists/
