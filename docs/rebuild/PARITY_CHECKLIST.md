...
| Debug tooling: 634, 635 | | Misc-screen 10-label debug selector (S12, D-023) + runtime debug toasts for all 10 %AAB_Debug categories via DebugSink/ToastDebugSink (S12.5c, G2-F15: LIGHT_EVAL/ANIMATION/DYNAMIC_SCALE/GRAPH_METRICS/SKIP_ANIMATIONS in the pipeline, SUPER_DIMMING in the coordinator, CONTEXT_AUTOMATION/LOCATION in ContextEngine); persistent in-app log view deferred (D-044) | ported (selector + runtime toasts; log view deferred) |
