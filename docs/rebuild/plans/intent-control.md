...
> content must by then live in `STATE.md` Changelog lines + the `D-156` ledger row. Code comments
> cite `D-156`, never this file.
...
**Security posture (goes into the D-156 row):** this deliberately re-opens the class of surface D-147
...
- **U2 — exported control receiver, core verbs (glue-review).** New `control/ControlReceiver.kt`
  ...
  register `exported="true"` + intent-filter + D-156 comment. `AutoBrightnessRuntime.panic`.
  Ledger row **D-156**. Tests `ControlReceiverTest`: `controlDisabled_ignoresAllActions_D156`
...
- **U6 — docs, release polish, plan retirement.** `README.md` automation recipes;
  `DEVICE_TEST_SCRIPT.md` §12 owner matrix; finalize `changelogs/18.txt`; `datastore_map.md`
  `control_prefs` row. **Delete this file.** Final `STATE.md` update (collapse Active work → Changelog
  lines). No `PARITY_CHECKLIST.md` row (Tasker-independent feature).
...
5. Commit (cite D-156; never the literal skip-ci token — D-115) and
