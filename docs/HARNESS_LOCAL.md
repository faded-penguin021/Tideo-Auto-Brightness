...
| `CITATION_EXCLUDE` | the two test paths | Their fixtures carry synthetic IDs by design. **Latent risk:** with `scripts/` in scope, a future upstream release that writes a bare `D-NNN` into a shipped script would turn the ladder red with no in-policy fix except excluding that script's whole path. All four shipped scripts contain zero matches today — upstream writes its own rows as `AMH ledger row D004` precisely to avoid this — so check it on an upgrade. |
