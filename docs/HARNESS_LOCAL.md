...
- **`comment-budget.sh`** — the constitution puts durable prose in the `.md` tier and leaves a
  ...
  **That floor started as a tree-wide count and the count was not enough.** `grep -c '// Tasker' >=
  ...
  `elements26`, `L15204`, `%AAB_*`); the manifest of 59 records lives in the guard, every record
  must still be present, and new records need no manifest edit because the manifest is a floor and
  not a whitelist. Keying on coordinates rather than on the marker's text is what makes it livable:
  22 of the 68 markers were reworded by the consolidation that shipped this guard, each keeping its
  reference while shortening the prose, and a text-keyed manifest would have gone red on all 22 —
  ...
  marker **re-pointed** at a different sub-action of the same task is not detected, and the 15
  records that cite no recognised coordinate degenerate to the old per-file population check.
...
`scripts/tests/local-guards.sh` is their fixture suite — 68 cases, run by `scripts/verify.sh`.
